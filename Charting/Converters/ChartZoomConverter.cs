	...
	public class ChartZoomConverter : IMultiValueConverter
	{
		public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
		{
				...
				&& values[2] is FrameworkElement grid && series.Any())
			{
				...
				var dataMin = series.Min(x => x.Values.Min(y => y.XValue));
				var dataMax = series.Max(x => x.Values.Max(y => y.XValue));

				var leftFrac = (zoom.XMin - dataMin) / Math.Max(dataMax - dataMin, 1);
				var rightFrac = (dataMax - zoom.XMax) / Math.Max(dataMax - dataMin, 1);
